...
pub fn process_reset_v2(accounts: &[AccountInfo<'_>], _data: &[u8]) -> ProgramResult {
    // Load accounts.
    let clock = Clock::get()?;
    let (godl_accounts, entropy_accounts) = accounts.split_at(15);
    sol_log(&format!("Godl accounts: {:?}", godl_accounts.len()).to_string());
    sol_log(&format!("Entropy accounts: {:?}", entropy_accounts.len()).to_string());
    let [signer_info, board_info, config_info, fee_collector_info, mint_info, round_info, round_next_info, _top_miner_info, sol_motherlode_info, treasury_info, treasury_tokens_info, system_program, token_program, godl_program, slot_hashes_sysvar] =
        godl_accounts
    else {
    ...
    let motherlode_denominator = config.motherlode_denominator.max(5);
